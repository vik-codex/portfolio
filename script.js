...
(function () {
  ...
  if (!el) return;
  setInterval(() => { el.textContent = 70 + Math.floor(Math.random() * 6); }, 1200);
})();
...
d.skills.forEach(cat => {
  ...
  card.innerHTML = `
    ...
    <div class="skill-tags">
      ${cat.items.map(item => `<span class="skill-tag" style="color:${cat.color};border-color:${cat.color}40;background:${cat.color}0f">${item}</span>`).join('')}
    </div>
  `;
...
d.experience.forEach(exp => {
  ...
  item.innerHTML = `
    ...
    <div class="timeline-card" style="border-left:3px solid ${exp.color}">
      ...
      <span class="timeline-type" style="color:${exp.color};border-color:${exp.color}45;background:${exp.color}12">${exp.type}</span>
...
// ── BUILD CERTIFICATIONS (clickable) ──────────────────────────────────────
...
function closeCertModalBtn() { closeModal(); }
