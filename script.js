/* ============================================================
   ...
   Runs synchronously after data.js — no event listeners needed.
   ============================================================ */
...
(function(){
  ...
  SKILLS.forEach(function(cat){
    ...
    cat.items.forEach(function(item){ html+='<span class="skill-tag">'+item+'</span>'; });
    html+='</div></div>';
  });
  grid.innerHTML=html;
})();
...
(function(){
  ...
  PROJECTS.forEach(function(p){
    ...
    html+='<div class="proj-title-row">';
    html+='<div class="proj-title">'+p.title+'</div>';
    if(p.github){ html+='<a href="'+p.github+'" target="_blank" class="proj-gh-link">GitHub →</a>'; }
    html+='</div>';
    ...
    if(p.outputs && p.outputs.length){
      html+='<div class="proj-outputs">';
      p.outputs.forEach(function(img,idx){
        var label=(p.outputLabels && p.outputLabels[idx])||('Output '+(idx+1));
        html+='<div class="proj-thumb-wrap" onclick="openLightbox(\''+escQ(img)+'\',\''+escQ(label)+'\')">';
        html+='<img src="'+img+'" alt="'+label+'" class="proj-thumb" onerror="this.parentElement.style.display=\'none\'"/>';
        html+='<div class="proj-thumb-label">'+label+'</div>';
        html+='</div>';
      });
      html+='</div>';
    }
...
(function(){
  ...
  JOB_SIMS.forEach(function(j){
    var clickAttr=j.img?'onclick="openModal(\''+escQ(j.img)+'\',\''+j.icon+'\',\''+escQ(j.role)+'\',\''+escQ(j.company)+'\',\''+j.period+'\')"':'';
...
function openLightbox(src, caption) {
  ...
  document.getElementById('m-name').textContent   = caption || '';
  document.getElementById('m-issuer').textContent = '';
...
/* ── SCROLL REVEAL ──────────────────────────────────────────── */
(function(){
  var els = document.querySelectorAll('.skill-card,.tl-item,.proj-card,.cert-card,.js-card,.about-grid,.contact-box');
